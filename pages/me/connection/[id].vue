    ...
    <nav class="sticky top-0 z-50 bg-white/80 backdrop-blur-md border-b border-stone-200">
      <div class="max-w-5xl mx-auto px-4 h-16 flex items-center justify-between">
        ...
        <NuxtLink to="/me" class="flex items-center gap-2 group">
          ...
          <span class="font-bold text-stone-900 tracking-tight hidden sm:block">minutes2match</span>
        </NuxtLink>
        
        <!-- User Info -->
        <div class="flex items-center gap-3">
          <div class="text-right hidden sm:block">
            <p class="text-sm font-bold text-stone-900">{{ matchProfile?.display_name || 'Connection' }}</p>
            <p class="text-xs text-stone-400">Your Match</p>
          </div>
          <div class="w-10 h-10 rounded-full overflow-hidden bg-stone-100 border-2 border-white shadow-sm">
            <img v-if="matchProfile?.photo_url" :src="matchProfile.photo_url" class="w-full h-full object-cover" />
            <div v-else class="w-full h-full flex items-center justify-center text-lg">{{ personaData?.emoji || '✨' }}</div>
          </div>
        </div>
      </div>
      ...
      <div class="max-w-5xl mx-auto px-4">
        <div class="flex gap-8 border-t border-stone-100">
          <NuxtLink 
            to="/me"
            class="py-3 text-sm font-bold tracking-wide uppercase transition-all whitespace-nowrap border-b-2 border-transparent text-stone-400 hover:text-stone-600"
          >
            ← Back to Matches
          </NuxtLink>
        </div>
    ...
    <div v-else class="max-w-6xl mx-auto px-4 py-8 pb-32">
      <div class="grid md:grid-cols-12 gap-8">
        ...
        <div class="md:col-span-4 lg:col-span-3 space-y-6">
          <!-- Profile Card -->
          <div class="bg-white p-6 rounded-2xl border border-stone-200 shadow-sm text-center sticky top-24">
            <!-- Photo -->
            <div class="relative inline-block mb-4">
              <div 
                class="w-32 h-32 rounded-full overflow-hidden border-4 border-stone-100 mx-auto"
              ...
              <!-- Badge -->
              <div 
                class="absolute -bottom-2 left-1/2 -translate-x-1/2 px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider border border-white shadow-sm"
                :class="match?.unlocked ? 'bg-emerald-500 text-white' : 'bg-black text-white'"
              >
            ...
            <p class="text-sm text-stone-500 mb-6 font-medium">
            ...
            <!-- Actions -->
            <div class="space-y-3">
              <div v-if="match?.unlocked && matchProfile" class="grid grid-cols-2 gap-3">
                <a 
                  :href="`https://wa.me/${matchProfile.phone?.replace(/\D/g, '')}`"
                  target="_blank"
                  class="flex flex-col items-center justify-center gap-1 p-3 bg-emerald-50 border border-emerald-100 text-emerald-700 hover:bg-emerald-100 rounded-xl transition-colors"
                >
                ...
                <a 
                  ...
                  class="flex flex-col items-center justify-center gap-1 p-3 bg-stone-50 border border-stone-200 text-stone-700 hover:bg-stone-100 rounded-xl transition-colors"
                >
              ...
              <button 
                ...
                @click="handleUnlock"
                :disabled="unlocking"
                class="w-full py-3 bg-black text-white rounded-xl font-bold text-sm shadow-sm hover:bg-stone-800 transition-colors disabled:opacity-50"
              >
        ...
        <div class="md:col-span-8 lg:col-span-9 space-y-6">
          ...
          <!-- Basic Details -->
          <div v-if="match?.unlocked && matchProfile" class="bg-white p-6 md:p-8 rounded-2xl border border-stone-200 shadow-sm">
             ...
             <div class="grid md:grid-cols-2 gap-6">
                 ...
                 <div class="space-y-2">
                    ...
                    <div class="w-full px-4 py-3 bg-stone-50 rounded-xl border border-stone-100 text-stone-900 font-medium">
                 ...
                 <!-- Phone -->
                 <div class="space-y-2">
                    <label class="text-xs font-bold uppercase text-stone-500 tracking-wide">Phone (Verified)</label>
                    <div class="w-full px-4 py-3 bg-stone-50 rounded-xl border border-stone-100 text-stone-900 font-medium">
                        {{ matchProfile.phone }}
                    </div>
                 </div>
                 ...
                 <!-- Religion -->
                 <div class="space-y-2">
                    <label class="text-xs font-bold uppercase text-stone-500 tracking-wide">Religion ⛪️</label>
                    <div class="w-full px-4 py-3 bg-stone-50 rounded-xl border border-stone-100 text-stone-900 font-medium">
                 ...
                 <!-- Height -->
                 <div class="space-y-2">
                    <label class="text-xs font-bold uppercase text-stone-500 tracking-wide">Height 📏</label>
                     <div class="w-full px-4 py-3 bg-stone-50 rounded-xl border border-stone-100 text-stone-900 font-medium">
                 ...
                 <!-- Intent -->
                 <div class="space-y-2">
                    <label class="text-xs font-bold uppercase text-stone-500 tracking-wide">Looking For 💍</label>
                     <div class="w-full px-4 py-3 bg-stone-50 rounded-xl border border-stone-100 text-stone-900 font-medium capitalize">
          ...
          <div v-if="match?.unlocked && matchProfile" class="bg-white p-6 md:p-8 rounded-2xl border border-stone-200 shadow-sm">
             <div class="flex items-center justify-between mb-4">
                 ...
                 <span class="text-xs text-stone-400">{{ matchProfile.about_me?.length || 0 }}/300</span>
             </div>
             <div class="w-full p-4 bg-stone-50 rounded-xl border border-stone-100 text-stone-700 min-h-[100px] leading-relaxed">
                {{ matchProfile.about_me || 'No bio available yet.' }}
             </div>
          </div>

          <!-- Interests -->
          <div v-if="match?.unlocked && matchProfile?.interests?.length" class="bg-white p-6 md:p-8 rounded-2xl border border-stone-200 shadow-sm">
             <div class="flex items-center justify-between mb-6">
                <h3 class="text-xl font-bold text-stone-900">Interests</h3>
                <span class="text-xs text-stone-400">{{ matchProfile.interests.length }} selected</span>
             </div>
             <div class="flex flex-wrap gap-2">
                <span 
                  v-for="interest in matchProfile.interests" 
                  :key="interest"
                  class="px-4 py-2 bg-stone-100 text-stone-700 rounded-full text-sm font-medium border border-stone-200"
                >
          ...
          <div v-if="match?.unlocked" class="bg-white p-6 md:p-8 rounded-2xl border border-stone-200 shadow-sm">
             <h3 class="text-xl font-bold text-stone-900 mb-4">Ice Breakers 🧊</h3>
             <div class="grid md:grid-cols-2 gap-3">
               <button 
                v-for="(starter, index) in conversationStarters" 
                :key="index"
                @click="copyStarter(starter, index)"
                class="group relative text-left p-4 pr-10 rounded-xl border border-stone-100 hover:border-black hover:bg-stone-50 transition-all text-sm font-medium text-stone-600 hover:text-stone-900"
              >
                {{ starter }}
                <div v-if="copiedIndex === index" class="absolute top-2 right-2 text-[10px] font-bold text-emerald-600 bg-emerald-50 px-2 py-1 rounded-full border border-emerald-100">
                  Copied!
                </div>
                <div v-else class="absolute top-3 right-3 opacity-0 group-hover:opacity-100 transition-opacity text-stone-400">
          ...
          <!-- Pro Tips (Bottom) -->
          <div class="bg-amber-50 p-6 md:p-8 rounded-2xl border border-amber-100 shadow-sm">
            <div class="flex items-start gap-4">
               <div class="pt-1">
                  <span class="text-2xl">💡</span>
               </div>
               <div>
                   <h3 class="text-lg font-bold text-stone-900 mb-2">Safety & Etiquette Tips</h3>
                   <ul class="space-y-2 text-sm text-stone-700">
                      ...
                      <li class="flex gap-2">
                        <span class="text-amber-500">•</span>
                        Reference their profile to show genuine interest.
                      </li>
                   </ul>
               </div>
            </div>
