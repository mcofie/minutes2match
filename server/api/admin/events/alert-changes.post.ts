...
export default defineEventHandler(async (event) => {
  ...
  // Fetch bookings that are active (confirmed, checked_in, pending)
  ...
  for (const booking of bookings) {
    ...
    try {
      ...
      console.error(`[Event Change Alert SMS] Failed to send to ${profile.phone}:`, err.message)
  ...
  if (historyToInsert.length > 0) {
    ...
    if (logError) {
      console.error('[Event Change Alert SMS] Failed to log history:', logError)
    }
  }
